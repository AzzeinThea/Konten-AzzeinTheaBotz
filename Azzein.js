...
azzein.on('group-participants-update', async (anu) => {
      ...
      if (anu.action == "add" && !num.includes(azzein.user.jid)) {
        ...
        imageMsg = (
          await azzein.prepareMessageMedia(buff, "imageMessage", {
            thumbnail: buff,
          })
        ).imageMessage;
        buttonsMessage = {
        ...
        prep = await azzein.prepareMessageFromContent(
          mdata.id,
          { buttonsMessage },
          {contextInfo: { "mentionedJid" : [num], "forwardingScore":999,"isForwarded":true},sendEphemeral: true}
        );
        azzein.relayWAMessage(prep);
      } 
       if (anu.action == "remove" && !num.includes(azzein.user.jid)) {
        let w = azzein.contacts[num] || { notify: num.replace(/@.+/, "") };
...
"inviteCode": "mememteeeekkeke","groupName": "azzeinbot", 
...
_Selamat jabatan kamu naik_`
			try {
				ppimg = await azzein.getProfilePicture(numm)
			} catch {
				ppimg = 'https://i.ibb.co/rvsVF3r/5012fbb87660.png'
			}
			let buff = await getBuffer(ppimg)
				azzein.sendMessage(mdata.id, buff, MessageType.image, {thumbnail: Buffer.alloc(0), quoted : falfa, caption: teks, contextInfo: { "mentionedJid" : [num], "forwardingScore":999,"isForwarded":true},sendEphemeral: true })
...
_Kasian malah kena demote_`
			...
			let buff = await getBuffer(ppimg)
				azzein.sendMessage(mdata.id, buff, MessageType.image, {quoted : falfa, caption: teks, contextInfo: { "mentionedJid" : [num], "forwardingScore":999,"isForwarded":true},sendEphemeral: true })
				}
     } catch (e) {
...
/*azzein.on('chat-update', async (anu) => {
	console.log(anu)
        falfa = { key: {fromMe: false,participant: "0@s.whatsapp.net",
remoteJid: "0@s.whatsapp.net"},message: {"groupInviteMessage": {"groupJid": "6288213840883-1616169743@g.us","inviteCode": "mememteeeekkeke","groupName": "azzeinbot", "caption": `Made With ❤️`, 'jpegThumbnail': `./image/${setting.thumbnail}`}}}
  metdata = await azzein.groupMetadata(anu.jid)
  try {
					ppimg = await azzein.getProfilePicture(anu.jid)
				} catch {
					ppimg = 'https://i.ibb.co/rvsVF3r/5012fbb87660.png'
				}
			let buff = await getBuffer(ppimg)
    ...
    azzein.sendMessage(metdata.id, buff, MessageType.image, {thumbnail: Buffer.alloc(0), caption: teks,contextInfo:{"forwardingScore":999,"isForwarded":true},sendEphemeral: true  , quoted: falfa})
...
antidel = true
azzein.on('message-delete', async (m) => {
...
azzein.sendMessage(m.key.remoteJid, ` *A N T I  D E L E T E*
 ‘Nama : @${m.participant.split("@")[0]}
 ‘Waktu : ${jam} ${week} ${calender}
 ‘Type : ${type}`, MessageType.text, {quoted: m.message, contextInfo: {"mentionedJid": [m.participant]}})

azzein.copyNForward(m.key.remoteJid, m.message)
})
antical = true
azzein.on("CB:Call", json => {
if (antical === false) return
let call;
calling = JSON.parse(JSON.stringify(json))
call = calling[1].from
azzein.sendMessage(call, `*Sorry ${azzein.user.name} can't receive calls.*\n*Call = Block!*`, MessageType.text)
.then(() => azzein.blockUser(call, "add"))
})
isBattre = 'Not Detect' 
isCharge = 'Not Detect' 
azzein.on (`CB:action,,battery`, json => {
                const batteryLevelStr = json[2][0][1].value
                const batterylevel = parseInt (batteryLevelStr)
                isBattre = batterylevel + "%"
                isCharge = json[2][0][1].live
})
azzein.on('CB:Blocklist', json => {
